...
func init() {
	Command = &cli.Command{
		Name:      "deeptable",
		Usage:     "CLI for the deeptable API",
		Suggest:   true,
		Version:   Version,
		...
		Flags: []cli.Flag{
			...
			&requestflag.Flag[string]{
				Name:    "api-key",
				Sources: cli.EnvVars("DEEPTABLE_API_KEY"),
			},
		},
		Commands: []*cli.Command{
			...
			},
			{
				...
				Suggest:  true,
				Commands: []*cli.Command{
					&structuredSheetsCreate,
					&structuredSheetsRetrieve,
					&structuredSheetsList,
					&structuredSheetsDelete,
					&structuredSheetsCancel,
					&structuredSheetsDownload,
				},
			},
			{
				Name:     "structured-sheets:tables",
				Category: "API RESOURCE",
				Suggest:  true,
				Commands: []*cli.Command{
					&structuredSheetsTablesRetrieve,
					&structuredSheetsTablesList,
					&structuredSheetsTablesDownload,
				},
			},
			{
				Name:            "@manpages",
				Usage:           "Generate documentation for 'man'",
				UsageText:       "deeptable @manpages [-o deeptable.1] [--gzip]",
				Hidden:          true,
...
func generateManpages(ctx context.Context, c *cli.Command) error {
	...
	if c.Bool("text") {
		file, err := os.Create(filepath.Join(dir, "man1", "deeptable.1"))
		if err != nil {
			return err
		}
		defer file.Close()
	...
	if c.Bool("gzip") {
		file, err := os.Create(filepath.Join(dir, "man1", "deeptable.1.gz"))
		if err != nil {
			return err
		}
		defer file.Close()
